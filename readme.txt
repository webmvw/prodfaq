=== ProdFAQ – Product FAQs for WooCommerce ===
Contributors: webmvw
Donate link: https://devsmasudrana.com
Tags: woocommerce, faq, product faq, accordion, product questions
Requires at least: 5.8
Tested up to: 6.4
Stable tag: 1.0.0
Requires PHP: 7.4
License: GPLv2 or later
License URI: https://www.gnu.org/licenses/gpl-2.0.html

== Description ==

ProdFAQ is a lightweight WooCommerce extension that allows you to add product-specific FAQs directly from the product edit screen.  
Display FAQs in a clean accordion layout on single product pages to improve customer experience and reduce pre-sale questions.

This plugin is built with simplicity, performance, and WordPress coding standards in mind.

== Features ==

* Add unlimited FAQs per WooCommerce product
* Simple and intuitive admin meta box UI
* Clean accordion-style frontend output
* SEO-friendly structure
* Lightweight and fast
* No external dependencies
* Fully translatable
* Developer-friendly and extendable

== Requirements ==

* WordPress 5.8 or higher
* WooCommerce must be installed and activated
* PHP 7.4 or higher

== Installation ==

1. Upload the `prodfaq` folder to the `/wp-content/plugins/` directory.
2. Activate the plugin through the **Plugins** menu in WordPress.
3. Make sure WooCommerce is installed and active.
4. Edit any WooCommerce product and find the **Product FAQs** meta box.
5. Add questions and answers, then update the product.

== Usage ==

After activating the plugin:

1. Go to **Products → Edit Product**
2. Scroll down to the **Product FAQs** meta box
3. Click **Add FAQ** to add a new question and answer
4. Save or update the product
5. FAQs will appear on the product’s single page automatically

== Frequently Asked Questions ==

= Does this plugin require WooCommerce? =
Yes. ProdFAQ works only with WooCommerce products.

= Can I add multiple FAQs per product? =
Yes, you can add unlimited FAQs per product.

= Does it work with all WooCommerce themes? =
ProdFAQ follows WooCommerce hooks and should work with most themes. Styling can be adjusted via CSS if needed.

= Is this plugin SEO-friendly? =
Yes. The FAQ content is output in clean HTML and can be extended with structured data if needed.

= Is the plugin translatable? =
Yes. The plugin is fully translation-ready using the `prodfaq` text domain.

== Screenshots ==

1. Product FAQ meta box in the WooCommerce product editor
2. Adding multiple FAQs in the admin panel
3. FAQ accordion display on the single product page

== Changelog ==

= 1.0.0 =
* Initial release
* Product-specific FAQ meta box
* Frontend FAQ accordion output
* WooCommerce dependency check

== Upgrade Notice ==

= 1.0.0 =
Initial stable release.

== Privacy Policy ==

ProdFAQ does not collect, store, or transmit any personal data.
